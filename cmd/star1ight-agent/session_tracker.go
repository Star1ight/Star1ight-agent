...
const sessionActivationBytes int64 = 32 * 1024
...
func (t *SessionTracker) Prepare(inbound, user, source string) (func(int64), func()) {
	if t == nil || user == "" {
	...
	activate := func(bytes int64) {
		if bytes <= 0 {
			return
		}
		if active.Load() || closed.Load() {
			return
		}
		if seen.Add(bytes) < sessionActivationBytes {
			return
		}
		mu.Lock()
		defer mu.Unlock()
		...
		release = t.Open(inbound, user, source)
		active.Store(true)
	}
...
func (c *trackedConn) UpstreamReader() any {
	return c.Conn
}

func (c *trackedConn) UpstreamWriter() any {
	return c.Conn
}
...
func (c *trackedPacketConn) UpstreamReader() any {
	return c.PacketConn
}

func (c *trackedPacketConn) UpstreamWriter() any {
	return c.PacketConn
}
